...
async def create_aggregated_provider_for_user(
    session,
    user_id: int | None,
    broker_accounts: list | None = None,
) -> AggregatedDataProvider:
    """Async variant that resolves data-provider keys from the DB first.

    Use this from any async route that has a session and a logged-in
    user. Each keyed provider is resolved via
    :func:`src.backend.data_providers.key_resolver.resolve_data_provider_key`
    so DB > .env priority is honored. Then delegates to
    :func:`create_aggregated_provider` with the resolved keys.

    Args:
        session: AsyncSession (required).
        user_id: Current user id, or ``None`` for system-level callers
            (in which case only .env keys apply).
        broker_accounts: Same as :func:`create_aggregated_provider`.
    """
    from src.backend.data_providers.key_resolver import resolve_data_provider_key

    keyed = ("alpha_vantage", "polygon", "twelve_data", "finnhub", "tiingo")
    api_keys: dict[str, str] = {}
    for pid in keyed:
        key, _source = await resolve_data_provider_key(session, user_id, pid)
        if key:
            api_keys[pid] = key
    return create_aggregated_provider(
