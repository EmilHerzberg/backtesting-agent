...
Run as::
    ...
    python -m backtesting_agent.cli --preset quick
    python -m backtesting_agent.cli --config path/to/config.yaml
    python -m backtesting_agent.cli --asset AAPL MSFT --strategy SMACrossover --trials 200
...
from backtesting_agent.config.logging import (
    ErrorReporter,
    TrialProgressCallback,
    setup_backtest_logging,
)
from backtesting_agent.config.presets import get_preset
from backtesting_agent.config.schema import BacktestFullConfig, load_config

logger = logging.getLogger(__name__)

# Strategy class name -> importable class mapping
_STRATEGY_MAP: dict[str, str] = {
    "SMACrossover": "backtesting_agent.strategies.sma_crossover.SMACrossover",
    "RSIMeanReversion": "backtesting_agent.strategies.rsi_reversion.RSIMeanReversion",
    "BollingerBreakout": "backtesting_agent.strategies.bollinger_breakout.BollingerBreakout",
    "MACDSignalCross": "backtesting_agent.strategies.macd_cross.MACDSignalCross",
    "MultiIndicator": "backtesting_agent.strategies.multi_indicator.MultiIndicator",
}
...
def run_pipeline(
    ...
    """
    from backtesting_agent.marketdata.provider import create_provider
    from backtesting_agent.marketdata.quality import DataQualityChecker
    from backtesting_agent.marketdata.windows import LookbackConfig
    from backtesting_agent.engine.optimizer import OptimizationConfig, optimize
    ...
    for symbol, df in symbol_data.items():
        ...
        for strategy_name in config.strategy.names:
            ...
            from backtesting_agent.results.store import ResultStore
            store = ResultStore()
