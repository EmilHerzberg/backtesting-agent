...
from backtesting_agent.indicators.base import suggest_params
...
def optimize(
    ...
    """
    # ATS-2004: in determinism mode an unseeded sampler still explores a
    # different trial sequence each run (TPE/Random have their own RNG), so
    # default to a fixed seed when the caller didn't supply one. This makes
    # the standalone reproducible-by-default whenever the frozen snapshot is
    # in use, without every config having to remember to set optuna.seed.
    effective_seed = config.seed
    if _determinism_mode_active() and effective_seed is None:
        effective_seed = 42
        logger.info(
            "ATS-2004: determinism mode — defaulting Optuna seed to %d.",
            effective_seed,
        )
    sampler = _create_sampler(config.sampler, seed=effective_seed)
