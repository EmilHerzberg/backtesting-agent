...
class BacktestConfig:
    ...
    # :mod:`backtesting_agent.event_gate`.
...
def _resolve_event_gate_forward_ref() -> None:  # pragma: no cover -- import shim
    from backtesting_agent.config.schema import EventGateConfig  # noqa: F401
    ...
    *,
    ...
    Args:
        ...
        gates_df: ATS-2080 — optional pre-loaded gate DataFrame (schema
            per :func:`backtesting_agent.event_gate.load_gates_for_backtest`).
...
def _preload_gates_blocking(
    ...
    Returns:
        ...
        :func:`backtesting_agent.event_gate.load_gates_for_backtest`.
        Empty when no gates exist or when the DB is unreachable.
    """
    import asyncio

    from backtesting_agent.event_gate import (
        _empty_gates_df,
    ...
    async def _load() -> pd.DataFrame:
        # Standalone build: no event-context client. ``load_gates_for_backtest``
        # ignores the client and returns an empty frame (gating disabled).
        return await load_gates_for_backtest(
            None, symbol, start_date, end_date,
        )
