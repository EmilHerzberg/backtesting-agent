...
**Phase 2+3 review (2026-07-06, [PHASE2-3-REVIEW-2026-07-06.md](./PHASE2-3-REVIEW-2026-07-06.md)):** 10-dimension multi-agent audit → **20 confirmed, 0 critical, 0 high** (the P2/P3 stat/data core holds). Fixes applied (branch `quant-review/phase2-3-review-fixes`): M32 on the research path (`_default_fetch` end-inclusive); live `/candidates` PENDING → honest marker; H24 survivorship surfaced in robustness (candidate.weaknesses both modes); M21 `errored_gate` plumbed through the gatekeeper facade + loop; Šidák docstring corrected (online scheme ≈17% FWER at 20 peeks, NOT 5%); OOS report counts deduped per hash (H16); requested-OOS-that-can't-init now RAISES (no silent in-sample); H8 test's false "clears clean" claim removed; + M22/M23 live-wiring gating tests. **KEY CAVEAT (honest re-scope): the market-data provider/cache layer where H21/H23/H22/M33/M34/M35 live is NOT wired into either live path** — the research loop uses raw `yf.Ticker().history()` and the CLI uses bare `YahooProvider`; `CacheManager`/`AggregatedDataProvider`/keyed providers have no live caller. Those fixes are correct but **pre-positioned for future wiring** (the raw research path refetches the full window each run, so H21's seam can't occur there). H18's Šidák correction is **within-run only** (per-run in-memory peek count); cross-run hold-out mining remains. Remaining review follow-ups (test-gating for loop-peek/M19-PathB/M35-endpoint; cache replace-window + non-atomic edge cases; wiring the provider layer) tracked as backlog.
