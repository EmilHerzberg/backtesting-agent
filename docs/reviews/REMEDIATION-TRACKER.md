...
| M46 | Med | mech | DONE (best-survivor DEFERRED) | `test_reporter_llm.py` | 5B-open: the LLM-facing OOS descriptor collapsed to a boolean (1 PASS+3 FAIL read "passed"; all-UNEVALUATED read "failed"). Now honest tri-state (none / inconclusive / mixed / passed / failed, digit-free) + a MISSING benchmark reads "benchmark unavailable" (was a false "beat buy-and-hold"). ⚠ DEFERRED (owner): the headline "best survivor by raw in-sample Sharpe" (report_generator.py:264) — redefining run-wide "best" to prefer validated/PASS survivors changes numbers, not just narration. |
...
| M49 | Med | mech | DONE (regime_failed excl. DEFERRED) | `test_loop.py` | 5B-open: the plateau watermark was fed the raw in-sample Sharpe of gate-FAILED / critic-REJECTED trials, so one overfit high-Sharpe reject pinned it high and made R4 abandon a productive asset. Only an accepted `candidate` now advances it. ⚠ minor DEFERRED (owner): also excluding `regime_failed` candidates for full consistency with M28. |
| M52 | Med | mech | DONE | `test_regime_window.py` | 5B-open: create-run validated only mode/window — `agent_mode`/`rigor` were free strings (silently coerced; a non-rule_based mode set a leakage marker then made 0 LLM calls) and budgets were unbounded. Added `Field` bounds (max_runs/max_seconds/target_candidates ≥1; max_eur ≥0) + enum validation for agent_mode/rigor. Persisting the *effective* (not requested) model/mode is covered by M43 (logged substitution) + H31 (model_id). |
