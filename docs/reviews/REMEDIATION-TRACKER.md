...
**Scoreboard:** in-scope for release = **95** (3 C + 32 H + 60 M). Backlog = 23 L (+ 29 N-notes). Done: **87 / 95** (1A-1E + 2A-2C + 3A-3B + 4A + 4B · 5A: H19/H20/M8-M12/M17/M57-M59 · 5B: H11/H13/M13-M18/M28/M60 · 5B-open: M27(honesty)/M29/M30/M31/M46/M48/M49/M52). **✅ All 3 criticals + all 32 highs fixed. Phases 1-5 + the 5A-gap + 5B-open remediations each adversarially reviewed. Remaining ≈ 8 (H32/FX + M36 currency + the last regime/OOS/director mediums' DEFERRED owner-decision halves) + the 23 Low backlog.**
...
| ID | Sev | Bucket | Status | Test | Note |
|----|-----|--------|--------|------|------|
| H19 | High | dec | DONE | `test_budgets_5a.py`, `test_loop.py::…stable_family_key` | FIXED: is_mutation computed before the prev-template update; caps keyed on the stable lineage ROOT (not per-call uuid), so the anti-brute-force caps bind. 5A-gap (BUDGET-1/TI-2 RESOLVED): added the loop-level gating test — drives research_loop with a same-template strategist + spy controller and asserts the stable-root key is used every iteration and the per-hypothesis counter accumulates (fails on pre-fix loop.py). |
| H20 | High | dec | DONE (subordinate) | `test_budgets_5a.py` | per-lineage counter resets on family switch → no longer a global ~100/day kill switch. 5A-gap (BUDGET-2): documented that on the live path the loop passes the SAME key as hypothesis_id AND lineage_id, so with defaults (25<100) the per-hypothesis cap always binds first and the per-lineage/day cap never independently fires (the "still binds within a family" claim only held under inflated test limits). Re-keying the two caps to distinct granularities = optional follow-up. |
...
| M17 | Med | dec | DONE | `test_optimizer_5a.py` | generate_strategy suggests from a FIXED indicator list + prunes conflicts post-hoc (was dynamic categorical → crashed trial 2). 5A-gap: M17-EDGE — post-hoc dedup could leave FEWER than min_indicators; now raises TrialPruned (+gating test). M17-TEST-NONDET — the survival test relied on random sampling to trigger the pre-fix crash; seeded the sampler. PATH-1 disposition (below) resolved: the generator is unwired → marked test-only + removed the inert `use_generator` flag. |
| M57 | Med | dec | DONE (honest-doc) | (docstring) | fingerprint API docstring downgraded to NOT-YET-ENFORCED. 5A-gap (DET-1): completed the downgrade — two sibling docstrings (`compute_h_loose`, `_data_snapshot_sha`) still described the CI check as operative present-tense; reworded to conditional. |
| M58 | Med | dec | DONE (honest-doc) | (docstring) | documented that the golden-hash CI gate SKIPS (scripts/golden absent). 5A-gap (DET-3): removed the orphaned `tests/golden/backtest_hashes.json` (FOREIGN monolith SHA + absent snapshot, read only by an always-skipping test — implied a live gate). Wiring the golden gate = backlog. |
| M59 | Med | dec | DONE (honest-doc) | (docstring) | apply_determinism_env docstring states the runtime-no-op limitation. 5A-gap (DET-2): `docs/TESTING-CONCEPT.md` still asserted "backtests are bit-exact" + listed RUN-6 determinism as a covered P1 test — softened to design-intent / NOT-YET-COVERED (only optimizer best_params reproducibility is enforced). Real fix = launcher/threadpoolctl (backlog). |

**5A-gap review (2026-07-07):** the slices NOT covered by the 5B review (budget H19/H20, generator M17/PATH-1, determinism honest-docs M57/M58/M59) got a focused 3-dimension audit → **10 confirmed, 0 refuted, NO highs**. The 5A fixes' substance held; the findings were honest-doc *incompleteness* (DET-1/2/3 — sibling docstrings, a docs file, and an orphaned golden artifact still implied enforced determinism), *dead code* (the unwired generator + inert `use_generator` flag), and *test-gating* (H19's loop wiring untested = TI-2, M17's non-deterministic gate). All remediated above. Residual (low, deferred): re-keying H20's two caps to distinct granularities (BUDGET-2), and wiring the golden-hash CI gate (M58 backlog).
...
| M30 | Med | test | DONE | `test_regime_holdout.py::…select_on_train_wiring…` | 5B-open: added the integration regression test — drives the REAL run_research split + strategist construction (spy executor records each backtest window; always-pass gatekeeper) and asserts the split is recorded, the full window is kept for the hold-out bound, selection ran on the TRAIN slice, and the hold-out ran on [train_end, window_end]. Passes on main (its value is regression protection: flipping run.py `window_end=train_we`→full window fails it). |
...
| M27 | Med | mech | DONE (honesty half; scaling DEFERRED) | `test_regime_holdout.py` | 5B-open: 1B/M26 warm-up prereq already satisfied. Honesty half FIXED — the too-thin `unvalidated` branch now always reports the observed `holdout_sharpe` (and `holdout_t` when ≥2 returns) instead of dropping the evidence; corrected the `_train_split` docstring (no per-strategy trade-frequency floor exists). ⚠ OWNER DECISION DEFERRED: scaling the fixed `VALIDATE_MIN_TRADES`=20 bar to the (short) slice length so `regime_validated` is reachable for low-frequency templates — a regime-semantics call (density anchor + whether a per-bar t-test is admissible). Do NOT lower the shared constant (OOS lockbox reuses it). |
...
| M29 | Med | mech | DONE | `test_regime_holdout.py` | 5B-open: decay `retained_fraction` was a bare ratio (guarded only vs ≤0), so a routine near-zero soft-gated in-regime Sharpe (~0.02) yielded `0.5/0.02=25.0` → "2500% retained". Now the ratio is computed only above a 0.10 Sharpe floor + clamped to [-2,2], and the signed `retained_delta` is ALWAYS emitted (meaningful when the ratio is undefined; recovers the dropped negative-edge info). Characterization-only (no control-flow consumer). |
| M31 | Med | mech | DONE | `test_persistence_3b.py`, `test_regime_window.py` | 5B-open: regime candidate metrics are measured on the TRAIN slice but the run exposed only the FULL window and the UI labeled the metrics with it (~25% too long). Exposed `train_end` on the `/state` response + the persisted DB row; relabeled the regime badge to "train {ws}→{train_end} · hold-out →{we}" when a split exists (full-window label kept when there is no split). Metrics unchanged — only their label. |
| M46 | Med | mech | DONE (best-survivor DEFERRED) | `test_reporter_llm.py` | 5B-open: the LLM-facing OOS descriptor collapsed to a boolean (1 PASS+3 FAIL read "passed"; all-UNEVALUATED read "failed"). Now honest tri-state (none / inconclusive / mixed / passed / failed, digit-free). Benchmark half — 5B-open-REVIEW (M46-BENCH-DEAD) found the first "benchmark unavailable" guard keyed on `buy_hold_return is None` was dead (the loop always builds 0.0); threaded an explicit `benchmark_available` flag executor→candidate→descriptor. ⚠ HONEST SCOPE (5B-open-RE-REVIEW M46-1): for a candidate that CLEARED the gates the benchmark is in practice always computable (the min-activity HARD ≥5-trade floor forces a multi-bar equity curve), so `benchmark_available` is DEFENSIVE hardening, not a beat/underperform correction for any reachable candidate — the reachable "unavailable" path remains the empty-dict/reload (`_bh is None`) case. Kept because honest + harmless; the parallel `critic.py` has-benchmark check has the same (also-unreachable) pattern. ⚠ DEFERRED (owner): the headline "best survivor by raw in-sample Sharpe" (report_generator.py) — redefining run-wide "best" to prefer validated/PASS survivors changes numbers, not just narration. |
| M48 | Med | mech | DONE | `test_loop.py` | 5B-open: the rule-based Director had no stop rule for a run of consecutive "skipped" iterations, so persistent budget/dedup skips spun to the T6 iteration backstop, burning one paid strategist call per spin. Added a `consecutive_skips` counter + Director R3b (`skip_breaker`=6): rotate to the next asset, or stop the last/only one. A single transient skip stays neutral (breaker>1). |
| M49 | Med | mech | DONE | `test_loop.py`, `test_regime_holdout.py` | 5B-open: the plateau watermark was fed the raw in-sample Sharpe of gate-FAILED / critic-REJECTED trials, so one overfit high-Sharpe reject pinned it high and made R4 abandon a productive asset. Only an accepted `candidate` advances it. 5B-open-REVIEW (F1) closed the deferred residual: a `regime_failed` candidate (overfit-by-definition, high in-sample Sharpe) took the candidate branch and re-entered the harm in regime mode — now excluded from the watermark too (gated test via the frozen-data harness). |
| M52 | Med | mech | DONE | `test_regime_window.py` | 5B-open: create-run validated only mode/window — `agent_mode`/`rigor` were free strings (silently coerced; a non-rule_based mode set a leakage marker then made 0 LLM calls) and budgets were unbounded. Added `Field` bounds (max_runs/max_seconds/target_candidates ≥1; max_eur ≥0) + enum validation for agent_mode/rigor. 5B-open-REVIEW (F2): mirrored the enum guard INSIDE run_research (a direct non-API caller could reach the leakage-marker path) and persist_snapshot now back-fills the EFFECTIVE agent_mode (create_run_row stored the requested one), so a full_ai→rule_based degrade is recorded as it ran. |
...
| PATH-1 | High | dec | DONE | `test_indicator_library_m18.py` | The pandas indicator LIBRARY (`RSIIndicator`/`ADXIndicator`, incl. the H11/library-M16 fixes) is reachable ONLY via `generate_strategy`→`DynamicStrategy`, which has NO production caller. The LIVE RSI copy is fixed separately (M16/PATH-2). 5A-gap RESOLVED: took the "mark test-only + delete the inert flag" option — removed `use_generator`/`max_indicators` from `StrategyConfig` + both example YAMLs (they were read nowhere, so `use_generator: true` was a silent no-op — PATH1-HONESTY-1), and marked `generator.py` EXPERIMENTAL / NOT-WIRED in its module docstring. |
...
| TI-2 | Med | test | DONE | `test_loop.py::…stable_family_key` | RESOLVED in 5A-gap (BUDGET-1): added the loop-level gating test that drives the fully-mocked research_loop and asserts the cap keys on the stable lineage root + accumulates — fails on pre-fix loop.py. |
